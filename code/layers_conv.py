...
class Conv2D(nn.Module):
    '''
    Abstract transformer for Convolution layer
    '''
    ...
    def get_height_out(self, h_in):
        return ((h_in + 2 * self.padding[1] - self.dilation[1] *
                 (self.kernel_size[1] - 1) - 1) // self.stride[1]) + 1

    def get_width_out(self, w_in):
        return ((w_in + 2 * self.padding[0] - self.dilation[0] *
    ...
    def forward(self, input):
        ...
        lx_out_ = torch.ones(size=(self.in_channels * h_in * w_in,
                                   self.out_channels * h_out * w_out)) * float('inf')
        for outchannel in range(self.out_channels):
            for inchannel in range(self.in_channels):
                ...
                weighted_window = torch.zeros(
                    size=(windows.shape[0], h_in * w_in))
                for i in range(weighted_window.shape[0]):
                    # -1 depicts neurons due to padding
                    id = windows[i] > -1
        ...
        assert torch.sum(lx_out_.isinf()) <= 0
...
class Objective(nn.Module):
    '''
    Abstract transformer for the final objective
    '''

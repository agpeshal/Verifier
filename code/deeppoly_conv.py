from layers_conv import modLayer, Objective, ReLU
import torch
...
class Model(nn.Module):
    ...
    def verify(self, config):
        '''
        Attempt to verify the input by calculating the lower bound.
        Update the slopes if any of the lower bound is negative
        '''
        lr = config.lr
        ...
        # Optimizer
        # If no crossing ReLU -> parameters empty!
        try:
        ...
        while True:
            ...
            if self.loss == 0:          # network verified for the input image!
                return True
